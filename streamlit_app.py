...
if transformation_choice == "30010085 宏酒樽 (夜)":
    ...
    if raw_data_file is not None and mapping_file is not None:
        ...
        # ---------------- 1) Load raw (sheet 'rsmulia' or first) ----------------
        xls = excel_file_safe(raw_bytes, engine=raw_eng)
        sheet_name = None
        ...
        # ---------------- 2) Parse product banner & rows ----------------
        # banner e.g. row 4: "產品條件:5000299628713 ~ 5000299628713"
        try:
        ...
        for r in range(len(df)):
            ...
            # Update current customer on lines like "客戶編號:A1A20143 [愛國-鳳林 ]"
            ...
            # Transaction lines: "銷貨" (positive) or "銷貨退回" (negative)
            if c0 in ("銷貨", "銷貨退回"):
        ...
        # ---------------- 3) Load mappings & build unique-only dictionaries ----------------
        cust_map = read_excel_safe(map_bytes, sheet_name="Customer Mapping", dtype=str, header=0, engine=map_eng)
        sku_map  = read_excel_safe(map_bytes, sheet_name="SKU Mapping",    dtype=str, header=0, engine=map_eng)
        ...
        df_m["Customer Code"] = df_m["CustomerCode_norm"].map(m_cust_f).fillna(
                                  df_m["CustomerCode_norm"].map(m_cust_g)
                                ).fillna("")  # leave blank if unmapped
    ...
    if raw_data_file is not None and mapping_file is not None:
        ...
        # ---------------- 1) Load raw (use 'Page 1' if present; else first sheet) ----------------
        xls = excel_file_safe(raw_bytes, engine=raw_eng)
        sheet_name = None
        for sn in xls.sheet_names:
        ...
        # ---------------- 2) Parse product blocks & movement rows ----------------
        def minguo_to_ymd(s):
            s = "" if s is None else str(s).strip()
            m = re.match(r"^(\d{3})/(\d{2})/(\d{2})$", s)
            if not m:
        ...
        for r in range(len(df0)):
            ...
            c2  = str(df0.iat[r, 2]).strip()  if pd.notna(df0.iat[r, 2]) else ""  # date (民國)
            c4  = str(df0.iat[r, 4]).strip()  if pd.notna(df0.iat[r, 4]) else ""  # 異動項目
            c7  = str(df0.iat[r, 7]).strip()  if pd.notna(df0.iat[r, 7]) else ""  # 單據號碼
            c9  = str(df0.iat[r, 9]).strip()  if pd.notna(df0.iat[r, 9]) else ""  # 可能是 "品名規格:"
            c10 = str(df0.iat[r, 10]).strip() if pd.notna(df0.iat[r,10]) else ""  # 客戶編號
            c13 = str(df0.iat[r, 13]).strip() if pd.notna(df0.iat[r,13]) else ""  # 客戶名稱
            q15 = pd.to_numeric(df0.iat[r, 15], errors="coerce") if (df0.shape[1] > 15) else None

            # Product header line: "產品編號: XXXXX"
            if c0.startswith("產品編號:"):
                m = re.search(r"產品編號[:：]\s*([A-Za-z0-9\-]+)", c0)
                if m:
                    current_prod_code = m.group(1).strip().upper()
                # product name often shows up as "品名規格: ..."
                if c9.startswith("品名規格"):
                    current_prod_name = re.sub(r"^品名規格[:：]\s*", "", c9).strip()
                continue

            # Sometimes product name appears on a separate row in col 9
            if c9.startswith("品名規格"):
                current_prod_name = re.sub(r"^品名規格[:：]\s*", "", c9).strip()

            # Movement rows: 銷貨（庫存） or 銷貨退回
            if c4 in ("銷貨（庫存）", "銷貨退回"):
                if pd.isna(q15):
                    continue
                qty = int(q15)
        ...
        # ---------------- 4) Unique-only dictionaries (prefer group, fallback to global) ----------------
        def unique_only_map(dfm, key_col, val_col, normalize=lambda s: s):
            tmp = dfm[[key_col, val_col]].dropna().copy()
            tmp["key"] = tmp[key_col].astype(str).map(normalize)
            tmp["val"] = tmp[val_col].astype(str).str.strip()
        ...
        # ---------------- 5) Apply mapping (non-forced; BLANK if unmapped) ----------------
        df_m = raw_df.copy()
        df_m["CustomerCode_norm"] = df_m["CustomerCode_ext"].map(norm_code)
        df_m["Customer Code"] = df_m["CustomerCode_norm"].map(m_cust_f).fillna(
                                   df_m["CustomerCode_norm"].map(m_cust_g)
        ...
        # ---------------- 6) Assemble final & aggregate duplicates ----------------
        final = pd.DataFrame({
            "Type": "INV",
            "Action": "U",
            "GroupCode": group_code,
    ...
    if raw_data_file is not None and mapping_file is not None:
        ...
        # ---------------- 1) Load raw (prefer sheet named "4", else first sheet) ----------------
        xls = excel_file_safe(raw_bytes, engine=raw_eng)
        sheet_name = None
        for sn in xls.sheet_names:
            if str(sn).strip() == "4":
        ...
        # ---------------- 2) Extract end date from banner like "14/07/01 - 14/07/31" or "114/07/01 - 114/07/31" ----------------
        def parse_end_date_from_banner(df):
            # scan first ~15 rows, first ~6 columns
            for r in range(min(15, len(df))):
                for c in range(min(6, df.shape[1])):
                    ...
                    if m:
                        ...
                        yy = int(y2)
                        if len(y2) == 3:   # Minguo year
                            year = yy + 1911
                        else:              # two-digit year -> assume 2000s
                            year = 2000 + yy
                        return f"{year:04d}{int(mm2):02d}{int(dd2):02d}"
                # join col0 + col1 if banner is split
                ...
                if m:
                    ...
                    if len(y2) == 3:
                        ...
                        year = 2000 + yy
                    return f"{year:04d}{int(mm2):02d}{int(dd2):02d}"
            return ""  # leave blank if not found

        date_end = parse_end_date_from_banner(df0)

        # ---------------- 3) Parse product blocks & transactional rows ----------------
        def parse_pair_to_bottles(s):
            # "箱數/瓶數" pair -> take bottles (the second number)
            s = str(s).strip()
        ...
        for r in range(len(df0)):
            ...
            c7  = str(df0.iat[r, 7]).strip()  if pd.notna(df0.iat[r, 7]) else ""  # 客戶代號
            c8  = str(df0.iat[r, 8]).strip()  if pd.notna(df0.iat[r, 8]) else ""  # 客戶名稱
            c9  = str(df0.iat[r, 9]).strip()  if pd.notna(df0.iat[r, 9]) else ""  # 銷售數量 pair a/b
            c10 = str(df0.iat[r,10]).strip()  if pd.notna(df0.iat[r,10]) else ""  # 銷退數量 pair a/b

            # Product header row: code in col0 (numeric), name in col1, spec in col2
            if re.match(r'^\d{6,}$', c0) and c1 and c2:
                current_prod_code = c0
                current_prod_name = c1  # keep simple; spec (c2) optional
                continue

            # Customer transactional row: has customer code & name; quantities appear as "a/b"
            if c7 and c8 and ("/" in c9 or "/" in c10):
                ...
                records.append({
                    "CustomerCode_ext": c7,
                    "CustomerName": c8,
                    "Date": date_end,              # leave blank if no banner found
                    "ProductCode": current_prod_code,
                    "ProductName": current_prod_name,
                    "Quantity": int(net)
                })
        ...
        # ---------------- 4) Load mappings (unique-only; BLANK if unmapped) ----------------
        cust_map = read_excel_safe(map_bytes, sheet_name="Customer Mapping", dtype=str, header=0, engine=map_eng)
        sku_map  = read_excel_safe(map_bytes, sheet_name="SKU Mapping",    dtype=str, header=0, engine=map_eng)
        ...
        df1["Customer Code"] = df1["CustomerCode_norm"].map(m_cust_f).fillna(
                               ...
                               ).fillna("")  # BLANK if still unmapped
        ...
        final = pd.DataFrame({
            ...
            "Date": df1["Date"],  # may be blank if banner not found
